...
cat configsMnist.txt | head -n 100 | while read CONFIG ; do
	...
	stack run --rts-options=-N$(nproc) -- mnist -R 20 -l "$LAYERS" -O "($OPTIM $LR $BETA1 $BETA2)" -s 100 -c "$COSTF" -I "$INPUTAF" > /tmp/results-mnist-$X.txt 

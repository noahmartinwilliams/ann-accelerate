...
data St = St { stNumEpochs :: Int, stBLInfo :: Maybe BLInfo, stTestPhase :: Bool, stStart :: Bool, stTrainFn :: Maybe TrainFn, stTestFn :: Maybe TestFn, stG :: StdGen, stTrainImgs :: [(Matrix Double, Matrix Double)], stTestImgs :: [(Matrix Double, Matrix Double)], stFileToOpen :: String, stOpenFile :: Handle, stFileToWrite :: String, stCloseFile :: Bool} deriving(Generic)
instance ToJSON Conf where
    toEncoding = genericToEncoding defaultOptions

defaultState :: StdGen -> Conf -> St
defaultState g (Conf { numEpochs = e}) = St { stNumEpochs = e, stBLInfo = Nothing, stTestPhase = False, stStart = True, stTrainFn = Nothing, stTestFn = Nothing, stG = g, stTrainImgs = [], stTestImgs = [], stFileToOpen = "", stOpenFile = stdout, stFileToWrite = "", stCloseFile = False }
